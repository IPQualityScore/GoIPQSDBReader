...
func (file *FileReader) Fetch(ip string) (*IPQSRecord, error) {
	...
	file_position := file.TreeStart
	if file.Version == 0x01 {
		file_position += int64(5)
	} else {
		file_position += int64(9)
	}
	...
	for l := 0; l < 257; l++ {
		...
		if literal[position] == "0" {
			...
			if file.Version == 0x01 {
				...
				file_position = int64(binary.LittleEndian.Uint64(read[8:16]))
			}
		}
	...
	for i := 0; i < len(file.Columns); i++ {
		...
		default:
			if c.Type.Has(StringData) {
				...
				value, err = getRangedStringValue(file, raw[current_byte:current_byte+stringsize])
				if err != nil {
...
func getRangedStringValue(file *FileReader, pointer []byte) (string, error) {
	position := uint64(binary.LittleEndian.Uint32(pointer))
	if file.Version == 0x02 {
		position = uint64(binary.LittleEndian.Uint64(pointer))
	}
	sizeraw := make([]byte, 1)
